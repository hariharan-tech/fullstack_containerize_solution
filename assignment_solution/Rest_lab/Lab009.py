...
def getEmployeeById(id):
    ...
    request = requests.get(f"http://localhost:8080/employees/{id}")
    # Check the URL to have employee ID passed

    # print the response content
    reqstr = json.dumps(request.json(),indent = 4)
    print(reqstr)
    # use json.dumps to convert response into string

    # refer reference to convert json string into a python object
    employeeObject = json.loads(reqstr,object_hook=lambda d: SimpleNamespace(**d))
